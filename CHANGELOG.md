...
- 2f00cc2: - **Scoped CSS Header & Variable Guidance**:
...
- d044462: - **Brand Modes Architecture (`themes` → `modes`)**:
...
- cfc0524: - **New Default Recursica Theme**:
...
- ee80dc9: - **1.x → 2.x Import Migration (tree & table)**:
  ...
  - **Dangling Font References Across Files**:
    ...
    - Runs on import _and_ in the store constructor, so every load path ends up referentially consistent — import, reset, fresh boot, and the bundle-version check that drops only the cached ui-kit and can otherwise leave an imported brand beside the bundled ui-kit.
...
- 81dea96: - **Structure Version 2.1.0 & Migration**:
...
- c435452: - **HoverCardPopover Preview Styling**:
  - Removed hardcoded paragraph styles (`pStyle`) from the `HoverCardPopoverPreview` component, allowing the component's design tokens and text properties to be applied and adjusted correctly.
  - **Component Preview & Layout**:
    - Enhanced the subtitle in `ComponentDetailPage` to display the active interaction state (e.g. "Base") instead of repeating the component's name when no custom variants or active states are selected.
...
- dfb3d54: - **Import Validation & Compatibility**:
...
- e6a4b67: Implemented a global undo/redo history stack for design tokens, themes, and UI kit overrides using fast-json-patch, debounced by 1 second to group active user interactions. Exposed via VarsContext and keyboard shortcuts (Cmd+Z/Ctrl+Z/Ctrl+Y).
...
- 46235e6: Update test exports to match latest UI Kit schema:
...
- ffb37ab: Fix randomizer token injection, resolve DTCG schema validation regressions, and restore Link text properties panel:
...
- ffb37ab: Fix theme token persistence, plural token specifications, and elevation variables bugs
...
- ef2b5c5: ### Slider Drag Performance & UI Polish
...
- c22e7ba: Fix font token pipeline: export variants, import sequence, reset, and delta conflicts
...
- f3cb980: **Fix: UIKit component changes no longer revert to defaults on export after a session reload**
...
- a974a65: ## Compliance: Suggest Tones — fix on-tone evaluation
...
- c743ce6: ## Font token architecture: dimension type for letter-spacing and line-height
