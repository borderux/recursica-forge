...
/**
 * Watch script for toolbar config JSON files
 * 
 * Automatically adds missing icons to iconLibrary.ts when toolbar config files change.
 * 
 * Usage: node scripts/watch-toolbar-icons.mjs
 */
...
const TOOLBAR_CONFIGS_DIR = join(projectRoot, 'src/modules/toolbar/configs')
 ...
 */
function getIconNamesFromToolbarConfigs() {
  const iconSet = new Set()
  
  try {
    const files = readdirSync(TOOLBAR_CONFIGS_DIR)
    const configFiles = files.filter(f => f.endsWith('.toolbar.json'))
    
    for (const file of configFiles) {
      const filePath = join(TOOLBAR_CONFIGS_DIR, file)
      try {
    ...
    console.error('Error reading toolbar configs directory:', error.message)
  }
...
function parseIconLibrary() {
  try {
    ...
    // Extract imports - handle both 'phosphor-react' and '@phosphor-icons/react'
...
function addMissingIcons() {
  const iconNames = getIconNamesFromToolbarConfigs()
  ...
  if (missingIcons.length === 0) {
    console.log('✓ All icons from toolbar configs are already in iconLibrary.ts')
    return
  }
 ...
 * Main watch function
 */
function watchToolbarConfigs() {
  console.log('👀 Watching toolbar config JSON files for changes...')
  console.log(`   Directory: ${TOOLBAR_CONFIGS_DIR}`)
  console.log('   Press Ctrl+C to stop\n')
  
  // Run once on startup
  addMissingIcons()
  
  // Watch for changes in the configs directory
  watch(TOOLBAR_CONFIGS_DIR, { recursive: false }, (eventType, filename) => {
    if (filename && filename.endsWith('.toolbar.json')) {
      console.log(`\n📝 ${filename} changed, checking for missing icons...`)
