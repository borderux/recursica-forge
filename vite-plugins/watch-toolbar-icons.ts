 ...
 */
function extractIconsFromConfig(config: any, iconSet: Set<string> = new Set()): Set<string> {
 ...
 */
function getIconNamesFromToolbarConfigs(configsDir: string): string[] {
 ...
 */
function parseIconLibrary(iconLibraryPath: string) {
  try {
    ...
    return { importedIcons: [], mappedNames: new Set<string>(), content: '' }
  }
}
 ...
 * Add missing icons to iconLibrary.ts
 */
function addMissingIcons(root: string) {
  ...
  const missingIcons: Array<{ iconName: string; componentName: string; needsImport: boolean }> = []
...
export function watchToolbarIcons(): Plugin {
  return {
