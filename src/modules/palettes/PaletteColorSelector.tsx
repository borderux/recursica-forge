...
export default function PaletteColorSelector({
  ...
  const recheckAACompliance = useCallback((family?: string) => {
    ...
    const tokensRoot: any = (tokensJson as any)?.tokens || {}
    const colorsRoot: any = tokensRoot?.colors || {}
    const resolveTokenName = (familyKey: string, level: string): string => {
      if (familyKey.startsWith('scale-')) {
        return `colors/${familyKey}/${level}`
      }
      // Check if it's an alias that maps to a scale
      for (const [scaleKey, scale] of Object.entries(colorsRoot)) {
        if (!scaleKey.startsWith('scale-')) continue
        if ((scale as any)?.alias === familyKey) {
          return `colors/${scaleKey}/${level}`
        }
      }
      return `color/${familyKey}/${level}`
    }
  ...
  }, [selectedFamily, mode, paletteKey, headerLevels, getTokenValueByName, setTheme, themeJson, tokensJson, getTokenLevelForPaletteLevel])
