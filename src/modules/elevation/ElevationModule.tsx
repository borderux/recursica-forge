...
export default function ElevationModule({ label, level, blurPx = 0, spreadPx = 0, offsetXPx = 0, offsetYPx = 0, colorHex = '#000000', alpha = 1, isSelected = false, onToggle, selectable = true, zIndex }: ElevationExampleProps) {
  ...
  const boxShadow = (() => {
    if (typeof level === 'number' && level >= 0) {
      ...
      return `var(--recursica-brand-light-elevations-elevation-${k}-x-axis) var(--recursica-brand-light-elevations-elevation-${k}-y-axis) var(--recursica-brand-light-elevations-elevation-${k}-blur) var(--recursica-brand-light-elevations-elevation-${k}-spread) var(--recursica-brand-light-elevations-elevation-${k}-shadow-color)`
    }
