...
export function updateCoreColorInteractiveOnTones(
  ...
  try {
    ...
    const findCoreColorInteractiveOnTone = (coreColorToneHex: string, coreColorName: string): string => {
      ...
      if (whiteContrast >= AA) {
        return `{brand.palettes.core-colors.white.tone}`
      }
      if (blackContrast >= AA) {
        return `{brand.palettes.core-colors.black.tone}`
      }
      ...
      return whiteContrast >= blackContrast
        ? `{brand.palettes.core-colors.white.tone}`
        : `{brand.palettes.core-colors.black.tone}`
    }
