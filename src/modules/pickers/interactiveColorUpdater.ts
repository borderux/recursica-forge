...
export function updateCoreColorOnTones(
  ...
  try {
    ...
    for (const colorName of coreColors) {
      ...
      let onToneCore: 'low-contrast' | 'high-contrast'
      if (whitePassesBoth && blackPassesBoth) {
        // Both pass - choose based on contrast
        onToneCore = whiteLowContrast >= blackLowContrast ? 'low-contrast' : 'high-contrast'
        ...
        onToneCore = whitePassesLow ? 'low-contrast' : 'high-contrast'
      } else if (whitePassesHigh || blackPassesHigh) {
        // At least one passes high emphasis
        onToneCore = whitePassesHigh ? 'low-contrast' : 'high-contrast'
      } else {
        ...
        onToneCore = whiteBaseContrast >= blackBaseContrast ? 'low-contrast' : 'high-contrast'
      }
