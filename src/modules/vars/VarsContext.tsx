...
export function VarsProvider({ children }: { children: React.ReactNode }) {
  ...
  // Apply Layer CSS variables from Brand.json and localStorage (regular and alternative)
  useEffect(() => {
    const applyLayerCss = () => {
      try {
        ...
        if (Object.keys(result).length) applyCssVars(result)
      } catch {}
    }

    applyLayerCss()
    const onTokens = () => applyLayerCss()
    const onReset = () => applyLayerCss()
    ...
    return () => {
      ...
      window.removeEventListener('paletteReset', onReset as any)
    }
  }, [tokens, theme])
