...
export function randomizeTransferList(componentNode: any, options: any, diffs: any[], pathPrefix: string): void {
    ...
    function isPathSelected(pathArr: string[]): boolean {
        ...
        const fullPathArr = ['transfer-list', ...pathArr];
        ...
        if ('$value' in node && typeof node.$value !== 'object') {
            ...
            if (typeof oldVal === 'string' && oldVal.startsWith('{')) {
                ...
                if (propName === 'text-decoration') {
                    ...
                    newVal = opts.filter(o => o !== oldVal)[Math.floor(Math.random() * (opts.length - 1))] || opts[0];
                } else if (propName === 'heading-level') {
