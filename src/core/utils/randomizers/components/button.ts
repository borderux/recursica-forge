...
export function randomizeButton(componentNode: any, options: any, diffs: any[], pathPrefix: string): void {
    ...
    function isPathSelected(pathArr: string[]): boolean {
        ...
        const fullPathArr = ['button', ...pathArr];
    ...
    function walk(node: any, path: string[]) {
        ...
        if ('$value' in node && (typeof node.$value !== 'object' || node.$value === null)) {
            ...
            if (newVal !== oldVal) { 
                node.$value = newVal; 
                diffs.push({ path: pathPrefix + '.' + path.join('.'), before: oldVal, after: newVal, changed: true }); 
                return;
            }
            return;
        }
