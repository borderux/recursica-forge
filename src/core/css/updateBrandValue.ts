...
import { getDelta, clearDeltaEntry } from '../store/cssDelta'
...
export function updateBrandValue(cssVar: string, value: string): boolean {
  ...
  // If the changed property is a core-color TONE (e.g. success.tone, alert.tone), any
  // compliance-written delta entries for that core-color reference the OLD scale and will
  // be re-applied by reapplyDelta() on the next recompute, overriding the user's change.
  // Clear stale delta entries for this core-color so the brand CSS var generated by
  // buildPaletteVars (from the updated state.theme) is not immediately overwritten.
  //
  // Detect core-color tone changes: path is [..., 'palettes', 'core-colors', colorKey, 'tone']
  const coreColorIdx = path.indexOf('palettes')
  if (
    coreColorIdx !== -1 &&
    (path[coreColorIdx + 1] === 'core-colors' || path[coreColorIdx + 1] === 'core') &&
    path[path.length - 1] === 'tone'
  ) {
    // Derive the CSS var prefix for this core-color in this mode.
    // e.g. --recursica_brand_themes_light_palettes_core-colors_success_
    const modeSeg = path[2] // 'light' or 'dark'
    const coreColorKey = path[coreColorIdx + 2] // e.g. 'success'
    const prefix = `--recursica_brand_themes_${modeSeg}_palettes_core-colors_${coreColorKey}_`
    const delta = getDelta()
    for (const varName of Object.keys(delta)) {
