...
describe('CSS Variable Propagation', () => {
  ...
  it('should propagate token changes to brand vars', () => {
    ...
    updateCssVar('--recursica_brand_modes_light_palettes_core-colors_high-contrast', 'var(--recursica_tokens_color_gray_500)')
    ...
    const brandVar = readCssVar('--recursica_brand_modes_light_palettes_core-colors_high-contrast')
    expect(brandVar).toBe('var(--recursica_tokens_color_gray_500)')
    
    // Resolved value should reflect new token value
    const resolved = readCssVarResolved('--recursica_brand_modes_light_palettes_core-colors_high-contrast')
  ...
  it('should handle nested var() references', () => {
    updateCssVar('--recursica_tokens_color_gray_500', '#808080')
    updateCssVar('--recursica_brand_modes_light_palettes_core-colors_high-contrast', 'var(--recursica_tokens_color_gray_500)')
    updateCssVar('--recursica_brand_light_layers_layer-0_properties_surface', 'var(--recursica_brand_modes_light_palettes_core-colors_high-contrast)')
  ...
  it('should maintain references when applying multiple vars', () => {
    const vars = {
      ...
      '--recursica_brand_modes_light_palettes_core-colors_high-contrast': 'var(--recursica_tokens_color_gray_500)',
      '--recursica_brand_light_layers_layer-0_properties_surface': 'var(--recursica_brand_modes_light_palettes_core-colors_high-contrast)'
    }
    
    applyCssVars(vars)
    
    expect(readCssVar('--recursica_tokens_color_gray_500')).toBe('#808080')
    expect(readCssVar('--recursica_brand_modes_light_palettes_core-colors_high-contrast')).toBe('var(--recursica_tokens_color_gray_500)')
    expect(readCssVar('--recursica_brand_light_layers_layer-0_properties_surface')).toBe('var(--recursica_brand_modes_light_palettes_core-colors_high-contrast)')
