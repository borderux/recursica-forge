        ...
        <Popover isOpen={pickerOpen} onClose={() => setPickerOpen(false)} content={timePickerPopoverContent} position="bottom" layer={layer} zIndex={9999}>
            <div className="recursica-time-picker-wrapper" style={{
                ...
                <input ref={inputRef} id={uniqueId} name={name} type="time" value={value} defaultValue={defaultValue} onChange={(e) => onChange?.(e.target.value)} onKeyDown={domProps.onKeyDown} onBlur={domProps.onBlur} placeholder={placeholder} disabled={state === 'disabled'} readOnly={domProps.readOnly} autoFocus={domProps.autoFocus} className={`recursica-time-picker-input ${className || ''}`}
                    style={{ flex: 1, minWidth: 0, maxWidth: '100%', padding: 0, border: 'none', backgroundColor: 'transparent', color: `var(${textVar})`, outline: 'none', textAlign: 'left', lineHeight: 'normal', fontSize: `var(${valueFontSizeVar}, 14px)` } as React.CSSProperties} {...carbon} />
                <style>{`
