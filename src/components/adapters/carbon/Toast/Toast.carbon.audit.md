# Toast Component Audit - Carbon

## Overview

This document audits the Toast component implementation for Carbon, identifying all CSS variables used, their sources, and any gaps in coverage.
...
|-----------------|-------------------|-------------------|--------|
| N/A | N/A | Carbon uses a simple div-based approach with Recursica CSS variables | ✅ Covered |

**Note**: Carbon CSS variables are not directly used. The component uses Recursica CSS variables with component-level custom properties that are then applied via CSS overrides.
...
The current implementation uses CSS custom properties set in TSX and referenced in CSS, which is the recommended pattern. All styling logic is properly separated between TSX (CSS variable setup) and CSS (style application).
