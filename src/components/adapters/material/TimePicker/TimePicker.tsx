/**
 * Material UI TimePicker Implementation
 * 
...
export default React.forwardRef<any, AdapterTimePickerProps & { labelId?: string; helpId?: string; errorId?: string }>(function TimePicker({
...
}: AdapterTimePickerProps & { labelId?: string; helpId?: string; errorId?: string }, ref) {
    ...
    const textVar = buildComponentCssVarPath('TimePicker', 'variants', 'states', effectiveState, 'properties', 'colors', layer, 'text-color')
    ...
    const labelGutterVar = layout === 'side-by-side' ? buildComponentCssVarPath('Label', 'variants', 'layouts', 'side-by-side', 'properties', 'gutter') : null
    ...
    if (layout === 'side-by-side' && labelElement) {
        ...
        return (<div ref={ref} className={`recursica-time-picker recursica-time-picker-side-by-side ${className || ''}`} style={style}><div style={{ display: 'flex', alignItems: 'flex-start', gap: gapValue, width: '100%' }}><div style={{ flexShrink: 0 }}>{labelElement}</div><div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>{fieldsRow}{assistiveElement}</div></div></div>)
    }
    return (<div ref={ref} className={`recursica-time-picker recursica-time-picker-stacked ${className || ''}`} style={style}><div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>{labelElement}{fieldsRow}{assistiveElement}</div></div>)
})
