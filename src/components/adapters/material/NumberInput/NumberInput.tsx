/**
 * Material UI NumberInput Implementation
 * 
 * Material UI-specific NumberInput component that uses CSS variables for theming.
...
export default function NumberInput({
    ...
    const focusBorderVar = buildComponentCssVarPath('NumberInput', 'variants', 'states', 'focus', 'properties', 'colors', layer, 'border-color')
    const focusBorderSizeVar = buildComponentCssVarPath('NumberInput', 'variants', 'states', 'focus', 'properties', 'border-size')

    // Get variant-specific border size
    const borderSizeVar = buildComponentCssVarPath('NumberInput', 'variants', 'states', effectiveState, 'properties', 'border-size')

    // Get component-level properties
    const borderRadiusVar = getComponentLevelCssVar('NumberInput', 'border-radius')
    const minHeightVar = getComponentLevelCssVar('NumberInput', 'min-height')
    const horizontalPaddingVar = getComponentLevelCssVar('NumberInput', 'horizontal-padding')
    ...
    const valueFontSizeVar = getComponentTextCssVar('NumberInput', 'text', 'font-size')
    const valueFontFamilyVar = getComponentTextCssVar('NumberInput', 'text', 'font-family')
    const valueFontWeightVar = getComponentTextCssVar('NumberInput', 'text', 'font-weight')
    const valueLetterSpacingVar = getComponentTextCssVar('NumberInput', 'text', 'letter-spacing')
    const valueLineHeightVar = getComponentTextCssVar('NumberInput', 'text', 'line-height')
    const valueTextDecorationVar = getComponentTextCssVar('NumberInput', 'text', 'text-decoration')
    const valueTextTransformVar = getComponentTextCssVar('NumberInput', 'text', 'text-transform')
    const valueFontStyleVar = getComponentTextCssVar('NumberInput', 'text', 'font-style')
    ...
    const inputWrapper = (
        <div
            className={`recursica-number-input-wrapper ${state === 'focus' ? 'focus' : ''}`}
        ...
        >
            {leadingIcon && (
                <div
                    className="recursica-number-input-leading-icon"
                    style={{
            ...
            <input
                ...
                className={`recursica-number-input-input ${className || ''}`}
                style={{
                    flex: 1,
                    minWidth: 0,
                    maxWidth: '100%',
            ...
            {trailingIcon && (
                <div
                    className="recursica-number-input-trailing-icon"
                    style={{
                        width: `var(${iconSizeVar}, 20px)`,
                        height: `var(${iconSizeVar}, 20px)`,
                        flexShrink: 0,
        ...
        #${uniqueId}.recursica-number-input-input {
        ...
        #${uniqueId}.recursica-number-input-input::placeholder,
        #${uniqueId}.recursica-number-input-input::-webkit-input-placeholder,
        #${uniqueId}.recursica-number-input-input::-moz-placeholder,
        #${uniqueId}.recursica-number-input-input:-ms-input-placeholder {
          color: var(${textVar}) !important;
          opacity: var(${placeholderOpacityVar}) !important;
        }
        #${uniqueId}.recursica-number-input-input:focus {
          /* Focus styles are handled by wrapper */
        }
        .recursica-number-input-wrapper:has(#${uniqueId}:focus),
        .recursica-number-input-wrapper.focus {
